...
(function () {
  ...
  // ── buildTurnPhaseStory (v03.01, compare-aware v03.06, ─────────
  //    colored-token sentences v03.10) ──────────────────────────
  // Returns a phase-specific sentence-hero spec for the four Turn
  // tabs (Approach / Wall / Underwater / Breakout).
  //
  // v03.10 — `sentence` is now a React node, not a string. Every
  // number is wrapped in a colored span: PRIMARY-trial values are
  // green (--lime-eff), COMPARE-related values (the verdict delta)
  // are purple (--compare-eff), matching the chart legend. Makes
  // "whose number is this" instant without reading. In compare
  // mode the self-context tail swaps for the verdict clause.
  //
  // Returns null when the phase isn't recognized; returns an
  // "unavailable" shape (plain-string sentence) when the canonical
  // column is missing, so the UI always renders something.
  function buildTurnPhaseStory(phase, primary, compare) {
    ...
    const G = (txt) => React.createElement('span',
      { style: { color: 'var(--lime-eff)' } }, txt);
    const P = (txt) => React.createElement('span',
      { style: { color: 'var(--compare-eff)' } }, txt);
    const F = (kids) => React.createElement(React.Fragment, null, ...kids);

    // Compare-target name. Benchmark holder names are never surfaced
    // (CLAUDE.md) — only the kind.
    const compareKind = compare ? (compare._benchmarkKind || 'TRIAL') : null;
    const targetName  = compareKind === 'PB'     ? 'your best'
                      : compareKind === 'MEDIAN' ? 'your median'
                                                  : 'compare';

    // verdict — Approach A comparative clause. Returns { mag, rest }
    // where mag is the compare-delta magnitude (rendered purple) and
    // rest is the directional phrase + target. null when nothing to
    // compare.
    ...
    if (phase === 'Approach') {
      ...
      const hl = v50.toFixed(2) + ' m/s';
      let subtext = 'Average velocity through the last 5 m before the wall.';
      if (v1510 != null) {
        const carry = +(v50 - v1510).toFixed(2);
        if (Math.abs(carry) < 0.03) subtext = 'Held speed steady from 15 m out into the wall.';
        else if (carry > 0) subtext = 'Accelerated ' + carry.toFixed(2) + ' m/s from the 15-10 m zone into the wall.';
        else subtext = 'Lost ' + Math.abs(carry).toFixed(2) + ' m/s from 15 m out to the wall.';
      }
      const cmp = compare ? n(compare, 'avg_vel_5_0_pre') : null;
      const d = cmp != null ? +(v50 - cmp).toFixed(2) : null;
      const v = verdict(d, 'higher', 'ahead of', 'behind', Math.abs(d || 0).toFixed(2) + ' m/s');
      const sentence = v
        ? F(['Carried ', G(hl), ' into the wall'].concat(tail(v)))
        : F(['Carried ', G(hl), ' through the last 5 m before the wall.']);
      return { sentence, subtext, delta: null };
    }

    if (phase === 'Wall') {
      // v03.01 — PUSH-OFF GAIN: velocity difference between the 5 m
      // before wall (avg_vel_5_0_pre) and 5 m after (avg_vel_0_5).
      ...
      let hl;
      if (gain > 0)      hl = '+' + gain.toFixed(2) + ' m/s';
      else if (gain < 0) hl = gain.toFixed(2) + ' m/s';
      else               hl = '±0 m/s';
      const verb = gain > 0 ? 'gained' : gain < 0 ? 'lost' : 'held';
      ...
      const d = cmpGain != null ? +(gain - cmpGain).toFixed(2) : null;
      const v = verdict(d, 'higher', 'stronger than', 'weaker than', Math.abs(d || 0).toFixed(2) + ' m/s');

      let sentence;
      if (v) {
        sentence = F(['Push-off ' + verb + ' ', G(hl)].concat(tail(v)));
      } else if (gain !== 0 && gainPct != null) {
        const word = gain > 0 ? 'boost over' : 'drop from';
        sentence = F(['Push-off ' + verb + ' ', G(hl), ' — a ',
                      G(Math.abs(gainPct) + '%'), ' ' + word + ' your approach.']);
      } else if (gain !== 0) {
        sentence = F(['Push-off ' + verb + ' ', G(hl), '.']);
      } else {
        sentence = F(['Push-off matched your approach speed exactly.']);
      }
    ...
    if (phase === 'Underwater') {
      ...
      if (kick != null) {
        const hl = kick.toFixed(1) + '/min';
        const subtext = sbr != null
          ? sbr.toFixed(2) + ' s from push-off to surface break.'
          : 'Kicks per minute between push-off and surface break.';
        const cmp = compare ? n(compare, 'kick_rate') : null;
        const d = cmp != null ? +(kick - cmp).toFixed(1) : null;
        const v = verdict(d, 'higher', 'above', 'below', Math.abs(d || 0).toFixed(1) + '/min');
        const sentence = v
          ? F(['Held ', G(hl), ' kick rate'].concat(tail(v)))
          : F(['Held ', G(hl), ' kick rate through the underwater.']);
        return { sentence, subtext, delta: null };
      }
      const hl = sbr.toFixed(2) + ' s';
      return {
        sentence: F(['Stayed underwater ', G(hl), ' from push-off to surface break.']),
    ...
    if (phase === 'Breakout') {
      ...
      const hl = t515.toFixed(2) + ' s';
      const subtext = sr != null
        ? 'Re-engaged at ' + sr.toFixed(1) + ' strokes/min after breakout.'
        : 'Time from 5 m before the wall to 15 m past it.';
      const cmp = compare ? n(compare, 'time_5in_15out_s') : null;
      const d = cmp != null ? +(t515 - cmp).toFixed(2) : null;
      const v = verdict(d, 'lower', 'faster than', 'slower than', Math.abs(d || 0).toFixed(2) + ' s');
      const sentence = v
        ? F(['Covered 5 m in / 15 m out in ', G(hl)].concat(tail(v)))
        : F(['Covered 5 m in / 15 m out in ', G(hl), '.']);
      return { sentence, subtext, delta: null };
    }
